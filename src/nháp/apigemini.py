...
GEMINI_API_KEY = "AIzaSyAv42WYzIpmW-ZChqMkX_b0Q4_PjZGkPKw"  # Thay bằng key từ https://aistudio.google.com/
...
def search_sentence(sentence):
    ...
    try:
        ...
        prompt = f"""
Search for the exact sentence: "{sentence}" on Google.
Return exactly 2 search results as a JSON list, each with keys: "title", "link", "snippet".
Only return JSON, no extra text. Links must be valid, accessible URLs starting with http or https (returning HTTP 200 status).
If no exact results are found, return the top 2 URLs from the Google search results for the sentence, with general links not specific to any topic.
Must return exactly 2 URLs.
Example:
[
  {{
