...
def fetch_other_type_file(url: str, type_file: str):
    try:
        ...
        if type_file.lower() == "pdf":
            ...
            text = ""
            sentences = []
            status = "Lỗi: Chưa hỗ trợ định dạng DOC (cần antiword/textract)."
        else:
            text = ""
            sentences = []
            status = f"Lỗi: Không nhận dạng được định dạng file '{type_file}'."
...
#         print("Câu cần kiểm tra:", sentence)
#         print("Có xuất hiện trong tài liệu không?:", has_plagiarism)
#     else:
